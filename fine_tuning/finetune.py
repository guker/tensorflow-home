# encoding=utf-8
"""
使用imagent alexnet做为预训练模型，进行kaggle的猫狗识别
模型地址： http://www.cs.toronto.edu/~guerzhoy/tf_alexnet/
训练第一个epoch，准确率达到95%，效果明显
...
parser.add_argument('-tf', '--train_file', action='store', type=str,
                    default='./train.txt', help='train file')
parser.add_argument('-vf', '--val_file', action='store', type=str,
                    default='./val.txt', help='validation file')
...
parser.add_argument('-fp', '--filewriter_path', action='store', type=str,
                    default='./filewriter', help='filewriter_path')
