...
从图1中可以看出，这个多线程程序的Wall time，![](http://chart.googleapis.com/chart?cht=tx&chl=\Large t_{wall}=P/3+S) $ t_{wall}=P/3+S $ 。它的CPU time， $ t_{cpu}=P+S $ 。容易知道，如果使用一个线程， $ t_{wall} = t_{cpu}=P+S $ 。用户使用3个线程需要等待的时间（即wall time）和使用一个线程需要等待的时间之比为(P+S)/ (P/3+S)<3 。这也就是使用3个线程带来的加速比。
